...
func Machines(clusterID string, config *types.InstallConfig, pool *types.MachinePool, osImage, role, userDataSecret string, capabilities map[string]string, useImageGallery bool, in *MachineInput) ([]machineapi.Machine, *machinev1.ControlPlaneMachineSet, error) {
	...
	for idx := int64(0); idx < total; idx++ {
		...
		provider, err := provider(platform, mpool, osImage, userDataSecret, clusterID, role, &azIndex, capabilities, useImageGallery, in)
		if err != nil {
...
func provider(platform *azure.Platform, mpool *azure.MachinePool, osImage string, userDataSecret string, clusterID string, role string, azIdx *int, capabilities map[string]string, useImageGallery bool, in *MachineInput) (*machineapi.AzureMachineProviderSpec, error) {
	var az string
	...
	if mpool.OSDisk.DiskType == "" {
		mpool.OSDisk.DiskType = "PremiumV2_LRS"
	}
